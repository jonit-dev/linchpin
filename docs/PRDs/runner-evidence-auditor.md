...
| 2 | Auditor role metadata and provider resolution | `scripts/linchpin.sh:248` resolves roles; `scripts/linchpin.sh:2473` preflights eligible roles | No auditor role | New role; existing resolver retained in Phase 2 | Remove auditor row; eligible preflight refuses while off remains usable. |
| 3 | Run-local sentence overrides | `skills/linchpin/SKILL.md:36` routes request to `scripts/linchpin.sh:2312`; `scripts/linchpin.sh:1608` announces it | Persistent-only assignment path for transient requests | Existing explicit persistent assignment retained; transient dispatch added in Phase 2 | Request auditor off, then verify repo configuration is byte-identical. |
| 4 | Runner state, events, operation receipts and Markdown projection | `scripts/linchpin.sh:3536` and `scripts/linchpin.sh:3554` delegate lifecycle to `scripts/runner.sh`; coordinator `skills/prd-swarm-coordinator/SKILL.md:171` invokes CLI | Reconstructed launch commands and model polling | Coordinator path delegates in Phase 3 | Remove runner delegation; resumed worker is not recovered and the live CLI test fails. |
