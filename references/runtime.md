...
| Author | `codex` | `gpt-5.6-sol` | `high` | `codex exec` | authoring a new PRD in `prd-creator` |
| Worker | `codex` | `gpt-5.6-luna` | `max` | `codex exec --sandbox danger-full-access` | implementation, repair, tests, conflict resolution |
| Reviewer | `codex` | `gpt-5.6-sol` | `high` | `codex exec --sandbox read-only` | one independent review per lane |
...
5. The sanctioned continuation is `codex exec resume <session-id>` for codex and
...
`exec resume` has no `--sandbox` flag, so a continued codex worker takes the
same access through `-c sandbox_mode`. Resuming without it drops the worker back
into the default sandbox mid-lane, where its next commit fails.
